...
Widget buildPlatform3DViewer({
  ...
  return MobileProduct3DViewer(key: key, configuration: configuration, bridge: bridge, autoRotate: autoRotate);
}
...
class _MobileProduct3DViewerState extends State<MobileProduct3DViewer> {
  ...
  void initState() {
    super.initState();
    _loadBundledAssets();
    _safetyTimeout = Timer(const Duration(milliseconds: 3000), () {
  ...
  Future<void> _loadBundledAssets() async {
    try {
      ...
      if (mounted) setState(() => _combinedHtml = inlined);
    } catch (e) {
    ...
    return Stack(
      children: [
        InAppWebView(
          ...
          onWebViewCreated: (controller) => widget.bridge.attachController(controller),
          ...
          onConsoleMessage: (controller, message) => debugPrint('[ThreeJS] ${message.message}'),
        ),
        if (_isLoading)
          const ColoredBox(
            color: AppColors.neutralSurface,
