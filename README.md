...
The whole agreed workflow is built and tested:

> **projects → door or window → material and colour → draw → confirm sizes and
> CH/Z → 2.5D preview → notes → save, reopen and export**

Nothing below describes a feature that does not exist. Where something is
partial or unverified, it says so and says why.

| Document | What it is |
| --- | --- |
| [SPEC.md](SPEC.md) | The requirements this was built to |
| [REQUIREMENTS.md](REQUIREMENTS.md) | Every requirement, its status, and the test that proves it |
| [USER_GUIDE.md](USER_GUIDE.md) | How to use the app, for the person drawing |
| [samples/](samples/) | A real project and its exports, generated by the app's own export path |
...
### Saving, projects and recovery

Projects are the app's front door: create, name, duplicate, reopen, delete —
with a confirmation that **names** what is about to be deleted.

**Writes are atomic.** A project is written to a scratch key first and only
then moved to its real one, so a save interrupted halfway leaves the previous
version intact rather than a half-written file. A test proves it by failing a
write mid-save and checking the last good version survives.

**Two save mechanisms, deliberately different.** Autosave writes a draft a
moment after every change and stays silent if it fails — interrupting a drawing
gesture to report a failed safety net would be worse than the risk. The real
save *does* report, because a user who believes their work is saved when it is
not will lose it.

**Old files are migrated, never rejected.** Schema 1 stored one note per panel
as a string; schema 2 stores a list with positions and visibility. An old
project is upgraded on load and saved forward. A project from a *newer* build
is refused, because this one cannot know what it would be dropping.

### Notes

Structured data on stable ids, never painted into an image. Several per panel,
each with its own position and its own show/hide.

**Positions are fractional**, so a label stays where it was put when the panel
is resized — an absolute position would drift outside a panel that got
narrower.

**Splitting and merging are resolved explicitly.** A note goes to whichever new
panel covers the place it was sitting, keeping its position relative to its new
home, and every move is reported to the user. Silently dropping a fabricator's
remark, or silently putting it on the wrong half, are both worse than saying
what happened.

**A note never changes the product.** Writing "make this 2 metres wide" resizes
nothing. A test asserts it.

### Exports

| Format | What it is | Reopens for editing? |
| --- | --- | --- |
| `.proframe` | The design itself | **Yes** |
| PDF | Drawing, dimensions, CH/Z legend, every note, warnings | No |
| PNG | The front view, dimensions and annotations optional | No |

The app says this plainly wherever exports appear. A PDF is a picture of a
design; only the project file is the design.

The PDF and the PNG share one geometry source (`FrontElevation`), so the two
drawings cannot drift apart. The sheet is labelled **"Not to scale — fitted to
the page"**, because a print scale is not implemented and claiming one would be
a lie.

**Unicode works properly.** Noto Sans and Noto Sans Arabic are bundled and used
for the interface, the PNG and the PDF. Before that, an Arabic note came out of
the PDF as a row of empty boxes — and so did an em dash.
...
- **It is a 2.5D preview, not a 3D engine.** No perspective, no lighting model.
  The camera raises, lowers and turns around; it does not orbit, because an
  axonometric projection has nothing to orbit through. The geometry is real and
  the projection is honest, but nothing here is a CAD kernel.
- **No curves.** Straight edges and straight sloping tops only. A curved stroke
  is discarded, never quietly straightened.
- **No localisation.** The interface is English. Arabic and Kurdish *text*
  works everywhere — notes, exports, the PDF — but the app's own labels are not
  translated.
- **No profile catalogue importer.** The interface for one is in place and a
  project records which system it wants, but swapping in supplier data still
  needs code. No catalogue has been supplied.
- **Dragging a note label is not wired up.** The model and the controller
  support moving one; no gesture calls it yet.
- **No pricing, inventory, CNC or CAD export.** Explicitly out of scope.
...
| `flutter test` | **300 tests, all passing** |
...
flutter test                 # the full suite, 300 tests
flutter build web --release  # verified here; output in build/web

# Not verified in this environment — see the table above
flutter build apk --release  # needs the Android SDK
flutter build ipa            # needs macOS and Xcode

# Regenerate the sample project and its exports
flutter test test/samples_generator_test.dart
```

The web build in `build/web` is a complete, installable artifact: serve that
directory over HTTP and the app runs. It is the one deployable output this
environment could actually produce.
...
| `test/infrastructure/persistence_test.dart` | Save, reopen, duplicate, drafts, interrupted saves, schema migration, the project file |
| `test/domain/layout/notes_and_validation_test.dart` | Note transfer on split/merge, and every dimension conflict |
| `test/integration/acceptance_test.dart` | The 16-step main scenario, plus the door and window workflows |
| `test/samples_generator_test.dart` | Regenerates `samples/` through the real export path |
